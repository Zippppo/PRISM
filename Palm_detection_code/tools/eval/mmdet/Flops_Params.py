...
def inference(args, logger):
    ...
    # 添加以下代码来禁用 checkpoint
    def turn_off_checkpoint(module):
        if hasattr(module, 'with_cp'):
            module.with_cp = False
        for child in module.children():
            turn_off_checkpoint(child)
    
    # 在评估之前关闭 checkpoint
    turn_off_checkpoint(model)
    model.eval()
    ...
    flops1, macs1, params1 = calculate_flops(
        model=model,
