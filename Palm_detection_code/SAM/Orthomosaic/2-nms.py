"""
Non-Maximum Suppression (NMS) for Object Detection Results

This script applies NMS to filter overlapping detections from the prediction results.
It processes the CSV files generated by the detection phase and produces filtered results.

Features:
- Applies NMS using IoU threshold
- Filters detections based on aspect ratio
- Handles multiple sites and models
...
def apply_nms_to_sites(site_numbers, model_name, iou_threshold=0.5):
    ...
    for site_number in site_numbers:
        ...
        csv_path = os.path.join("v10results", f'predictions_{tif_name[:-4]}_{model_name}.csv')
        output_csv_path = os.path.join("v10results", f'filtered_{tif_name[:-4]}_{model_name}.csv')
